<?php
  ...
  $foto_t;
	...
	$foto_v = "visual/".$_SESSION['email'].'_'.basename ($_FILES ['imagem_visual']['name']);
	}
	else{
		return -1;
	}
	...
	if (isset ($_FILES ['imagem_tatil']) && $_FILES ['imagem_tatil']['name'] != '') {
		$foto_t = "tatil/".$_SESSION['email'].'_'.basename ($_FILES ['imagem_tatil']['name']);
	}
	else{
		return -1;
	}
